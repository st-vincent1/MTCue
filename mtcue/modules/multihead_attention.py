...
class ScaleUp(nn.Module):
    """https://github.com/CyndxAI/QKNorm/blob/c628cb5d21f1475ba95db779a175748ff9efe940/QKNorm/layers.py#L8"""
...
class MultiheadAttention(nn.Module):
    ...
    def __init__(
        ...
        self.qknorm = qknorm
        if self.qknorm:
            assert temperature is not None, "Provide temperature for qk norm"
            self.scaling = ScaleUp(temperature)
        else:
            self.scaling = self.head_dim ** -0.5
        # Scaling is set to work with QK norm
    ...
    def forward(
        ...
        if self.encoder_decoder_attention:
            if self.qknorm:
                q, k = map(l2norm, (q, k))
                q = q.view((kv_bsz, self.num_heads) + q.size()[1:])
                k = k.view((kv_bsz, self.num_heads) + k.size()[1:])
                attn_weights = self.scaling(torch.einsum("bhid,bhjd->bhij", q, k))
            else:
            ...
            if self.qknorm:
                q, k = map(l2norm, (q, k))
                attn_weights = self.scaling(torch.einsum('bid,bjd->bij', q, k))
            else:
        ...
        # filler = float("-inf")
        filler = -torch.finfo(attn_weights.dtype).max
        if key_padding_mask is not None:
            ...
            if not is_tpu:
                ...
                attn_weights = attn_weights.masked_fill(key_padding_mask, filler)
                attn_weights = attn_weights.transpose(0, 2)
            attn_weights = attn_weights.view(bsz * self.num_heads, tgt_len, src_len)
